...
Core detects an installed provider when `--provider` is omitted. Set `WEBMCPIFY_PROVIDER` when you want a fixed default. Antigravity generation runs in its edit-acceptance mode inside Core's disposable workspace, so a user-level plan mode cannot leave a no-op draft.
